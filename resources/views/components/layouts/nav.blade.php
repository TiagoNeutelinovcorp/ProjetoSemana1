                ...
                @auth
                    <div class="dropdown dropdown-end">
                        ...
                        <ul tabindex="-1" class="dropdown-content menu bg-base-100 rounded-box z-[1] w-52 p-2 shadow-lg mt-2 border border-base-200">
                            ...
                            @if(auth()->user()->two_factor_secret)
                                <li>
                                    <a href="{{ route('chat.index') }}" class="flex items-center gap-2">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
                                        </svg>
                                        Chat
                                        @php
                                            $unreadCount = \App\Models\ChatMessage::where('user_id', '!=', auth()->id())
                                                ->whereHas('chatRoom.participantes', function($q) {
                                        ...
                                        @endphp
                                        @if($unreadCount > 0)
                                            <span class="badge badge-error badge-sm ml-auto">{{ $unreadCount }}</span>
                                        @endif
                                    </a>
                                </li>
                            @else
                                <li>
                                    <button onclick="chatModal.showModal()" class="flex items-center gap-2 w-full text-left hover:bg-base-200 p-2 rounded-lg">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
                                        </svg>
                                        Chat
